...
const HWaves: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const setLines = () => {
      ...
      const totalPoints = Math.ceil(oWidth / xGap);
    ...
    const movePoints = (time: number) => {
      lines.forEach((points) => {
        points.forEach((p) => {
          const move = noise.perlin2((p.x + time * 0.005) * 0.0015, (p.y + time * 0.0125) * 0.002) * 12;
          p.wave.x = Math.cos(move) * 16;
          p.wave.y = Math.sin(move) * 32;
